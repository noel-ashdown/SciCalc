# SciCalc â Scientific Calculator for Autodesk Fusion
...
SciCalc reads geometry values directly from your active selection â edges, faces, bodies
and sketch entities â and lets you use those values straight in your calculations.
...
- Inline expression editing â click anywhere in the expression to position the cursor
- Bracket balance indicator and scientific notation input (e.g. `1.5e6`)
- `Ctrl+Z` undo, `â` `â` arrow history recall
...
- **Settings** â theme, font size, decimal places, compact mode

### Geometry Panels
| Panel | Description |
|-------|-------------|
| **Triangle Solver** | Enter any 3 known values (sides a, b, c / angles A, B, C) â solves the full triangle |
| **Circle Geometry** | Enter any one value â computes radius, diameter, area, circumference, arc length, sector area |
| **Beam Section Properties** | Solid rectangle, hollow rectangle, solid circle, hollow circle â area, I, S, r |
| **Spring Calculator** | Helical compression spring â rate, spring index, Wahl factor |
| **Bolt / Thread Data** | M1âM36 metric and common UNC sizes â pitch, minor diameter, stress area |

### Conversion Panels
| Panel | Description |
|-------|-------------|
| **Unit Conversions** | 12 categories: length, area, volume, mass, temperature, force, pressure, angle, speed, energy, torque, power |
| **DMS â Decimal** | Degrees / minutes / seconds conversion |
| **Polar â Rectangular** | Coordinate conversion using current angle mode |
...
### Selection Values
Automatically reads the active Fusion selection every 0.4 seconds and displays:

**3D Model**
- Edge â length, radius, diameter
- Face â area, cylinder dimensions (radius, diameter, height), sphere, torus, cone
- Body â bounding box, volume, surface area, mass, centre of mass
- Vertex â world X, Y, Z coordinates
- Two vertices / points â distance and midpoint
- Two edges â angle between them
- Two faces â face normal angle and dihedral angle (or distance if parallel)

**Sketch**
- Line â length
- Circle â radius, diameter, circumference
- Arc â radius, diameter, arc length, sweep angle
- Spline â length, start / end world coordinates
- Conic curve â length, rho, start / end / apex world coordinates
- Point â sketch X/Y and world X/Y/Z
...
4. Go to **Tools â Scripts and Add-Ins** (or press `Shift+S`)
...
2. Copy the `SciCalc` folder to:
   ...
   ```
3. Follow steps 3â8 from the Windows instructions above

### Updating
Replace `SciCalc.py`, `calculator.html`, `help.html` and the `icons` folder with the new versions.
Do **not** replace `history.json`, `variables.json`, `settings.json` or `bookmarks.json` â these contain your saved data.

### Uninstalling
1. Go to **Tools â Scripts and Add-Ins â Add-Ins**, select SciCalc and click **Stop**
...
| File | Description |
|------|-------------|
| `SciCalc.py` | Add-in entry point â registers toolbar button and reads Fusion selection |
